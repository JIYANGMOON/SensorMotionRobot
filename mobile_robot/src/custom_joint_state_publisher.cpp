...
public:
    CustomJointStatePublisher() {
        if(!n.getParam("/custom_joint_state_publisher/joint_names", joint_names))
            throw std::runtime_error("set joint_names");
        if(!n.getParam("/custom_joint_state_publisher/joint_positions", joint_positions)) 
            throw std::runtime_error("set joint_positions");
